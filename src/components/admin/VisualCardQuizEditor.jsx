...
const VisualCardQuizEditor = ({ section, onSave, onCancel }) => {
  ...
  const addQuestion = () => {
    setQuestions([
      ...questions,
      { text: 'ما هو معنى الحرف في هذه الجملة؟', options: ['خيار 1', 'خيار 2', 'خيار 3'], correct: 0, hint: '', explanation: '' }
    ]);
  };
  ...
  return (
    <div className="bg-white p-4 md:p-6 rounded-xl border border-indigo-100 shadow-sm mt-4 animate-in fade-in slide-in-from-top-4 max-w-full">
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
        <div>
          <h3 className="text-xl font-bold text-indigo-900">محرر كارد كويز ✨</h3>
          <p className="text-sm text-slate-500">عدل البطاقات والخيارات (٣ خيارات لكل بطاقة)</p>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-8">
          <div className="flex items-center justify-between">
            <h4 className="font-bold text-slate-800">قائمة البطاقات ({questions.length})</h4>
          ...
          {questions.map((q, qIdx) => (
            <div key={qIdx} className="bg-white p-6 rounded-2xl border-2 border-slate-100 shadow-sm relative group hover:border-indigo-200 transition-colors">
              <div className="absolute top-3 left-3 flex gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity z-10">
                <button onClick={() => moveQuestion(qIdx, -1)} disabled={qIdx === 0} className="text-slate-300 hover:text-indigo-600 disabled:opacity-0" title="تحريك لأعلى"><ArrowUp size={20} /></button>
                <button onClick={() => moveQuestion(qIdx, 1)} disabled={qIdx === questions.length - 1} className="text-slate-300 hover:text-indigo-600 disabled:opacity-0" title="تحريك لأسفل"><ArrowDown size={20} /></button>
                <button onClick={() => removeQuestion(qIdx)} className="text-slate-300 hover:text-rose-500 transition-colors" title="حذف البطاقة"><Trash2 size={20} /></button>
              </div>
              <div className="space-y-4">
                <div>
                  <label className="block text-slate-600 text-xs font-bold mb-2 uppercase tracking-wider flex items-center gap-2">
                    <HelpCircle size={14} /> نص البطاقة {qIdx + 1}
                  </label>
                  <textarea 
                    value={q.text} 
                    onChange={(e) => handleQuestionChange(qIdx, 'text', e.target.value)} 
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 text-lg font-medium h-24"
                  ></textarea>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {q.options.map((opt, oIdx) => (
                    <div key={oIdx} className="relative flex flex-col gap-2">
                      <div className="flex items-center gap-2">
                        <button 
                          type="button"
                          onClick={() => handleQuestionChange(qIdx, 'correct', oIdx)}
                          className={`shrink-0 w-8 h-8 flex items-center justify-center rounded-full border-2 transition-all ${q.correct === oIdx ? 'bg-emerald-500 border-emerald-600 text-white shadow-md' : 'bg-white border-slate-200 text-slate-300 hover:border-indigo-300'}`}
                          title="تعيين كإجابة صحيحة"
                        >
                          <CheckCircle2 size={18} />
                        </button>
                        <span className="text-xs text-slate-400 font-bold">الخيار {oIdx + 1}</span>
                      </div>
                      <textarea 
                        value={opt} 
                        onChange={(e) => handleOptionChange(qIdx, oIdx, e.target.value)}
                        className={`w-full px-4 py-3 rounded-xl border-2 transition-all h-20 ${q.correct === oIdx ? 'border-emerald-200 bg-emerald-50/30 text-emerald-900' : 'border-slate-100 focus:border-indigo-400 text-slate-700'}`}
      ...
      <div className="flex justify-end gap-3 mt-8 pt-6 border-t border-slate-100">
        ...
        <button onClick={handleSave} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 md:px-10 py-2 rounded-xl font-bold shadow-md transition-all active:scale-95 text-sm md:text-base">اعتماد التغييرات</button>
      </div>
    </div>
  );
};

export default VisualCardQuizEditor;

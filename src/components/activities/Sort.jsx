    ...
    <div className="max-w-4xl mx-auto pb-6">
      ...
      <div className="flex flex-col items-center w-full gap-6 fade-in">
        <div className="relative w-full min-h-[15rem] transition-all duration-500 ease-in-out flex flex-col items-center justify-start pt-4 px-4 pb-8 md:px-6">
          ...
          <div className={`w-full max-w-2xl bg-white p-5 md:p-8 rounded-[2rem] shadow-lg border-2 border-slate-200 leading-snug flex flex-col justify-start z-10 ${animatingOut ? 'fly-out' : 'fade-in'} ${shakeClass}`}>
            ...
            <div className="w-full min-h-[14rem] md:min-h-[16rem] flex flex-col items-center justify-center mb-2">
          ...
          <div className={`w-full max-w-4xl mt-8 flex flex-wrap justify-center gap-4 ${disablePanelClass}`}>
            {currentQuestion.options.map((opt, optIdx) => {
              ...
              let btnClass = `flex-1 min-w-[140px] flex items-center justify-center min-h-[4.5rem] px-4 py-3 rounded-2xl font-bold text-lg md:text-xl transition-all duration-300 border-b-4 text-center leading-relaxed shadow-sm `;
              
              if (!animatingOut && status !== 'correct') {
                btnClass += `bg-white border-${btnColor}-300 text-${btnColor}-700 md:hover:bg-${btnColor}-50 md:hover:border-${btnColor}-500 md:hover:-translate-y-1 active:scale-95 cursor-pointer`;
              } else {
                btnClass += `bg-slate-50 border-slate-200 text-slate-400 opacity-60 cursor-default shadow-none`;
              }
